...
export function auditHtml(r: AuditReport, publicUrl?: string): string {
  ...
  const lines = [
    ...
    `scanned ${r.scanned.transfers} token transfers, ${r.scanned.nativeTxs} native txs${r.scanned.capped ? " (capped)" : ""}; inspected ${r.scanned.counterpartiesInspected}/${r.scanned.counterpartiesTotal} counterparties`,
    "",
    `<b>Track 2 signals</b>`,
    ...
    `x402 settlements: ${m.x402Settlements} · named stablecoins: ${m.stablecoins.length ? esc(m.stablecoins.join(", ")) : "none"} · USA₮ over x402: ${yn(m.usatOverX402)}`,
